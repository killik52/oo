...
import database.dao.FaturaNotaDao
import database.dao.FaturaFotoDao
import database.dao.InformacoesEmpresaDao
import database.dao.InstrucoesPagamentoDao
...
class SecondScreenActivity : AppCompatActivity() {
    ...
    private lateinit var faturaItemDao: com.example.myapplication.database.dao.FaturaItemDao
        ...
        if (success) {
            currentPhotoPath?.let { path ->
                lifecycleScope.launch(Dispatchers.IO) {
                    try {
                        val faturaFotoEntity =
                            FaturaFotoEntity(faturaId = currentFaturaId, photoPath = path)
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val fatura = Fatura(
                ...
                data = SimpleDateFormat(
                    "yyyy-MM-dd HH:mm:ss",
                    Locale.getDefault()
                ).format(Date()), // Sempre salva a data/hora atual da modificação
            ...
            withContext(Dispatchers.Main) {
                if (pdfFile != null) {
                    ...
                    val emailIntent = Intent(Intent.ACTION_SEND).apply {
                        type = "com/example/myapplication/application/pdf"
    ...
    private fun getFaturaFromUI(): Fatura {
        ...
        return Fatura(
            ...
            data = SimpleDateFormat(
                "yyyy-MM-dd HH:mm:ss",
                Locale.getDefault()
            ).format(Date()), // Use a data atual ao gerar o PDF
            observacao = null,
            ...
            try {
                ...
                if (faturaOriginal != null) {
                    ...
                    val faturaParaLixeira = FaturaLixeira(
                        ...
                        fotosJson = Gson().toJson(
                            faturaFotoDao.getPhotosForFatura(currentFaturaId).firstOrNull()
                                ?.map { it.photoPath }), // Salva paths das fotos
                        notasJson = Gson().toJson(
                            faturaNotaDao.getNotesForFatura(currentFaturaId).firstOrNull()
                                ?.map { it.notaConteudo }) // Salva conteúdo das notas
                    )
